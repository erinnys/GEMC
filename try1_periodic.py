...
class model:
    ...
    def initconfig(self):
        self.config=[]
        con=np.random.rand(len(self.latvec),self.tot)*self.latvec
        ac=0
        for i in self.num:
    ...
    def picture(self,step):
        if len(self.latvec)==3:
            ...
            ax.set_yticklabels([])
            plt.show()
        if len(self.latvec)==2:
            plt.figure()
            plt.title('step : {}'.format(step))
            for i in range(len(self.config)):
                plt.scatter(self.config[i][0],self.config[i][1],label=self.lab[i])
            plt.legend()
            plt.xlim(0,self.latvec[0])
            plt.ylim(0,self.latvec[1])
    ...
    def checkconfig(self):
        ...
        for i in range(len(self.config)):
            for j in range(len(self.config[i].T)):
                if (self.config[i].T[j]>self.latvec.T-self.rad).any():
                    chek=chek+1
